...
import numpy as np
from sheet01.tests.utils import (
    is_list_of_floats,
    is_positive_integer,
...
class BaseBanditEnv(ABC):
    ...
    def __init__(self, mean_parameter: List[float], max_steps: int) -> None:
        ...
        assert is_list_of_floats(
            mean_parameter
        ), "The means of a multiarmed bandit model should be a list of floats."
        assert isinstance(
            max_steps, int
        ), "The number of maximal steps should be an int."
        ...
        self.optimal: Tuple[float, float] = [
            max(mean_parameter),
...
class GaussianBanditEnv(BaseBanditEnv):
    ...
    def __init__(
        self, mean_parameter: List[float], max_steps: int, var_scale: float = 1.0
    ) -> None:
        """create a multiarm bandit with `len(p_parameter)` arms
...
class BernoulliBanditEnv(BaseBanditEnv):
    ...
    def __init__(self, mean_parameter: List[float], max_steps: int):
        super().__init__(mean_parameter=mean_parameter, max_steps=max_steps)
