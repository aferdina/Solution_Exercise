...
gauss_multi_arm = GaussianBanditEnv(mean_parameter=MEAN_PARAMETER, max_steps=MAX_STEPS)


def train_exandcommit(
    agent: ExploreThenCommit,
    env: GaussianBanditEnv,
    num_games: int,
    parameter: str,
    printed: bool,
) -> Tuple[List[float], List[int], List[float], List[float]]:
    """Start the explore then commit algorithm

    Args:
        agent (ExploreThenCommit): Explore then commit agent
        env (GaussianBanditEnv): multi armed environment to use the agent
        num_games (int): number of games to play the algorithm
        parameter (str): name of the parameter to be used
        printed (bool): bool if printed or not

    Returns:
        Tuple: rewards, chosen armsm regrets, optimalities
    """
