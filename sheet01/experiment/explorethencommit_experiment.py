...
def explorethencommit_exp(max_steps, n_arms, used_explores, num_games, printed):
    ...
    if printed:
        ...
        for exploration, traj in statistics_regrets.items():
            plt.plot(range(len(traj)), traj,
                     label=f"regrets, explore_steps {exploration}")
            plt.legend()
        plt.subplot(4, 1, 4)
...
if __name__ == "__main__":
    explorethencommit_exp(max_steps=MAX_STEPS, n_arms=N_ARMS,
                          used_explores=USED_EXPLORES, num_games=NUM_GAMES, printed=True)
