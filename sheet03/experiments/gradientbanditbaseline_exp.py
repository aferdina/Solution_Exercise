...
from sheet03.models.multiarmedmodels import GradientBandit
...
def gradientbandit_exp(max_steps, n_arms, used_alpha, num_games, printed):
    statistics_mean = {}
    statistics_cumsum = {}
    statistics_regrets = {}
    ...
    for alpha in used_alpha:
        ...
        for game in range(num_games):
            mean_parameter = np.random.normal(
                loc=4.0, scale=1.0, size=n_arms).tolist()
...
if __name__ == "__main__":
    statistic_mean, statistics_cumsum, statistics_regrets, statistics_optimalities = gradientbandit_exp(
        max_steps=MAX_STEPS, n_arms=N_ARMS, used_alpha=USED_ALPHA, num_games=NUM_GAMES, printed=True)
    ...
    with open("results/gradient_bandit.json", "w") as file:
